...
public final class z
{
    ...
    public z(InputStream inputStream) {
        ...
        try {
            ...
            int n2 = -10;
            bufferedInputStream2.read(b, 0, 3);
            if (b[0] == 73 && b[1] == 68 && b[2] == 51) {
                bufferedInputStream2.read(b, 0, 3);
                bufferedInputStream2.read(b, 0, 4);
                n2 = (b[0] << 21) + (b[1] << 14) + (b[2] << 7) + b[3];
            }
            n = n2 + 10;
        }
    ...
    private int a(final byte[] b, int off, int i) {
        int n = 0;
        try {
            while (i > 0) {
                final int read;
                if ((read = this.k.read(b, off, i)) == -1) {
                    while (i-- > 0) {
                        b[off++] = 0;
                    }
                    break;
    ...
    private int b(final byte[] b, int off, int i) {
        int n = 0;
        try {
            while (i > 0) {
                final int read;
